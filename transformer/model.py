...
class TracingTransformerEmbedderWrapper(nn.Module):
    def __init__(
            ...
            is_distributed):
        super().__init__()
        if is_distributed and not isinstance(time_text_embed.timestep_embedder.linear_1, ColumnParallelLinear):
...
class TracingTransformerBlockWrapper(nn.Module):
    def __init__(self, transformer, transformerblock, is_distributed):
        super().__init__()
        if is_distributed and not isinstance(transformerblock[0].attn.to_q, ColumnParallelLinear):
            for block_idx, block in enumerate(transformerblock):
